...
- [ ] D5 - Functional Build Flow (in progress; approved plan -> proposed diff and scheduler continuation exist, executor-owned PatchDraft loop still missing)
  ...
  - [x] ~~The mounted PatchDraft path now calls the scheduler-step bridge with only run/project/model/clock inputs; React no longer passes PatchDraft approval IDs, supported-test hints, plan files, plan steps, roots, or project path.~~
...
- [ ] D7 - Model Integration Depth (in progress; Ollama is real, OpenAI-compatible remains out of live scope)
  ...
  - [ ] Add a direct Anthropic route only as explicit opt-in model depth with keyring-backed secrets, approval-visible cloud boundary state, health checks, and deterministic route tests.
