...
impl ModelRegistry {
    ...
    pub fn with_default_local(checked_at: u64) -> Self {
        ...
        registry
            .providers
            .push(openai_compatible_missing_key(checked_at));
        let route_result =
            registry.save_role_route(ModelRole::Coding, "mock-local", "delyx-mock-coder");
    ...
    pub fn mock_complete(
        ...
        let route = self
            .route_for(role)
            .ok_or(ModelProviderError::ModelNotFound)?;
    ...
    fn provider(&self, provider_id: &str) -> Result<&ModelProvider, ModelProviderError> {
        self.providers
            ...
            .ok_or(ModelProviderError::ProviderNotFound)
    }
}
...
fn mock_provider(checked_at: u64) -> ModelProvider {
    ModelProvider {
        ...
        health: ProviderHealth {
            ...
            message: "Deterministic local mock provider is ready.".to_string(),
            checked_at,
        },
        models: vec![
            model("delyx-mock-coder", "Delyx Mock Coder", true),
            model("delyx-mock-reasoner", "Delyx Mock Reasoner", false),
        ],
...
fn ollama_provider(checked_at: u64) -> ModelProvider {
    ModelProvider {
        ...
        health: ProviderHealth {
            ...
            message: "No local Ollama endpoint configured yet.".to_string(),
            checked_at,
        },
        models: Vec::new(),
        secret_policy: SecretPolicy::NoSecretRequired,
...
fn stable_prompt_score(prompt: &str) -> u32 {
    prompt.bytes().fold(0_u32, |score, byte| {
        score.wrapping_mul(31).wrapping_add(byte as u32)
    })
}
