...
fn insert_file(
    ...
    connection
        .execute(
            "INSERT INTO patch_proposal_files (proposal_id, file_index, path)
             VALUES (?1, ?2, ?3)",
            params![proposal_id, file_index as i64, file.path],
        )
        .map(|_| ())
        .map_err(sql_string)
}
...
fn load_files(connection: &Connection, proposal_id: &str) -> Result<Vec<PatchFileView>, String> {
    let mut statement = connection
        .prepare(
            "SELECT file_index, path FROM patch_proposal_files
