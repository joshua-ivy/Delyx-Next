...
mod tests {
    ...
    use crate::external_agent::{
        AdapterStatus, ExternalAgentBridge, ExternalAgentCapturePlan, ExternalAgentError,
    ...
    fn detects_external_agent_adapters() {
        ...
        assert!(adapters
            .iter()
            .any(|adapter| adapter.adapter_id == "codex-cli"));
        assert!(adapters
            .iter()
            .any(|adapter| adapter.adapter_id == "claude-code"));
        assert!(adapters
            .iter()
            .any(|adapter| adapter.adapter_id == "generic-terminal"
    ...
    fn detects_external_agent_executables_from_path() {
        ...
        let codex = adapters
            .iter()
            .find(|adapter| adapter.adapter_id == "codex-cli")
            .unwrap();
        let claude = adapters
            .iter()
            .find(|adapter| adapter.adapter_id == "claude-code")
            .unwrap();
    ...
    fn external_agent_cannot_run_without_approval() {
        ...
        assert_eq!(
            result.unwrap_err(),
            ExternalAgentError::Approval(ApprovalError::NotApproved)
        );
    }
    ...
    fn external_agent_scope_must_be_inside_approved_root() {
        ...
        let result = bridge.run_approved_worker(
            run_request(&approval.id, &outside, true, vec![]),
            10,
            &approvals,
        );
