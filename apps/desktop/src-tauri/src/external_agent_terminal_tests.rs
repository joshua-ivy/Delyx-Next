...
mod tests {
    ...
    use crate::external_agent::{
        ...
        ExternalAgentRunRequest, ExternalAgentRunStatus, ExternalAgentScope,
    ...
    fn approved_generic_worker_runs_terminal_command_and_captures_output() {
        ...
        approvals
            .approve(&terminal.id, 10, "approved in test")
            .unwrap();
        let mut bridge = ExternalAgentBridge::new(vec![root.clone()]).unwrap();

        let artifact = bridge
            .run_approved_worker(
                command_request(&approval.id, &terminal.id, &root, passing_command()),
                10,
                &approvals,
            )
            .unwrap();
    ...
    fn failed_generic_worker_command_is_captured_as_failed_artifact() {
        ...
        let artifact = bridge
            .run_approved_worker(
                command_request(&approval.id, &terminal.id, &root, failing_command()),
                10,
                &approvals,
            )
            .unwrap();
        ...
        assert!(artifact
            .transcript
            .iter()
            .any(|event| event.kind == ExternalAgentEventKind::Failed));
    }
    ...
    fn worker_command_requires_terminal_command_tool() {
        ...
        assert_eq!(
            result.unwrap_err(),
            ExternalAgentError::ToolNotAllowed("terminal_command".to_string())
        );
    }
    ...
    fn worker_command_requires_terminal_command_approval() {
        ...
        let result = bridge.run_approved_worker(
            command_request(&approval.id, "", &root, passing_command()),
            10,
            &approvals,
        );
    ...
    fn command_request(
        ...
        request
            .task_policy
            .allowed_tools
            .push("terminal_command".to_string());
    ...
    fn passing_command() -> (String, Vec<String>) {
        if cfg!(windows) {
            (
                "cmd".to_string(),
                vec!["/C".to_string(), "echo worker passed".to_string()],
            )
        } else {
            (
                "sh".to_string(),
    ...
    fn failing_command() -> (String, Vec<String>) {
        if cfg!(windows) {
            (
                ...
                vec![
                    "/C".to_string(),
                    "echo worker failed 1>&2 & exit /B 7".to_string(),
                ],
            )
        } else {
            (
                ...
                vec![
                    "-c".to_string(),
                    "echo worker failed >&2; exit 7".to_string(),
                ],
            )
        }
    }
    ...
    fn terminal_command_input() -> ProposalInput {
        ProposalInput {
            ...
            expected_result: "Capture external worker stdout, stderr, exit status, and duration."
                .to_string(),
