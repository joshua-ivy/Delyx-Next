...
mod tests {
    ...
    fn automation_engine_survives_sqlite_reload() {
        ...
        let waiting = engine
            ...
            .unwrap();
        let blocked = engine
            .schedule_due_run(&contract.id, "changed", 11, &mut approvals)
            .unwrap();
        ...
        assert_eq!(
            loaded.contracts()[0].allowed_tools,
            vec!["terminal_command"]
        );
        assert_eq!(loaded.scheduled_runs()[0].id, waiting.id);
        assert_eq!(
            loaded.scheduled_runs()[0].status,
        ...
        assert_eq!(
            loaded.scheduled_runs()[1].status,
        ...
        let next_run = loaded
            .schedule_due_run(&contract.id, "changed", 12, &mut approvals)
            .unwrap();
