...
pub(crate) fn dirty_count(root: &Path, git_dir: &Path) -> Option<usize> {
    ...
    let tracked = entries.iter().map(|entry| entry.path.clone()).collect::<HashSet<_>>();
    let tracked_changes = entries.iter().filter(|entry| entry_is_dirty(root, entry)).count();
...
fn entry_is_dirty(root: &Path, entry: &GitIndexEntry) -> bool {
    ...
    entry.modified.is_some_and(|stamp| modified_stamp(&metadata) != Some(stamp))
}

fn count_untracked(root: &Path, current: &Path, tracked: &HashSet<String>) -> std::io::Result<usize> {
    let mut count = 0;
...
fn read_index(path: &Path) -> std::io::Result<Vec<GitIndexEntry>> {
    ...
    for _ in 0..count {
        ...
        let modified = Some((u64::from(read_u32(&data, offset + 8)?), read_u32(&data, offset + 12)?));
        ...
        entries.push(GitIndexEntry { modified, path, size });
    }
    Ok(entries)
}
