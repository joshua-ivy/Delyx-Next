...
check(rootPackage.version === "0.1.0", "root package version must match the dev desktop baseline");
check(desktopPackage.version === "0.1.0", "desktop package version must match the dev desktop baseline");
check(tauriConfig.productName === "Delyx Next", "Tauri productName must be Delyx Next");
check(tauriConfig.version === "0.1.0", "Tauri version must match the dev desktop baseline");
...
check(tauriConfig.app?.windows?.[0]?.theme === "Dark", "primary desktop window must request native dark theme");
...
check(bundleIcons(tauriConfig).includes("icons/32x32.png"), "bundle icon list must include 32x32.png");
check(bundleIcons(tauriConfig).includes("icons/128x128.png"), "bundle icon list must include 128x128.png");
check(bundleIcons(tauriConfig).includes("icons/128x128@2x.png"), "bundle icon list must include 128x128@2x.png");
check(bundleIcons(tauriConfig).includes("icons/icon.icns"), "bundle icon list must include mac desktop icon.icns");
...
check(cargoToml.includes('version = "0.1.0"'), "Cargo package version must match the dev desktop baseline");
check(existsSync(join(desktopRoot, "src-tauri", "src", "release.rs")), "support bundle release module must exist");
check(existsSync(join(desktopRoot, "src-tauri", "app-icon.svg")), "editable source icon SVG must exist");
check(existsSync(join(desktopRoot, "src-tauri", "icons", "32x32.png")), "desktop 32x32 icon must exist");
check(existsSync(join(desktopRoot, "src-tauri", "icons", "128x128.png")), "desktop 128x128 icon must exist");
check(existsSync(join(desktopRoot, "src-tauri", "icons", "128x128@2x.png")), "desktop 128x128@2x icon must exist");
check(existsSync(join(desktopRoot, "src-tauri", "icons", "icon.icns")), "desktop icon.icns must exist");
